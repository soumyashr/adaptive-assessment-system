...
const AdaptiveAssessment = () => {
  ...
  const [username, setUsername] = useState(''); // Simplified: just username
  ...
  // Default competence level (always start at beginner)
  const DEFAULT_COMPETENCE_LEVEL = 'C1';
  ...
  // FIXED: Updated generateDifficultyData to properly handle assessment completion
  const generateDifficultyData = useCallback((responses, currentDifficulty, currentTheta, isComplete) => {
    ...
    // FIXED: Only add current unanswered question if assessment is NOT complete
    ...
    console.log('Scatter data generated:', {
      isComplete,
      hasCurrentQuestion: !isComplete && currentDifficulty !== null,
  ...
  // SIMPLIFIED Login function - always uses default competence level
  const login = async () => {
    if (!username.trim()) {
    ...
    try {
      const response = await fetch(`${API_BASE}/users/`, {
        ...
        body: JSON.stringify({
          ...
          competence_level: DEFAULT_COMPETENCE_LEVEL // Always use C1 for new users
        })
      });
  ...
  // FIXED: Submit answer with proper state cleanup on completion
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (updatedSession.completed) {
          ...
          // FIXED: Clear current question difficulty when assessment completes
  ...
  // SIMPLIFIED Login Screen - removed competence level dropdown
  if (showLogin) {
    return (
        ...
        <div className="bg-gray-800 backdrop-blur-sm rounded-xl p-8 w-full max-w-md shadow-2xl border border-gray-700">
          ...
          <div className="space-y-6">
            ...
            <div className="text-center text-xs text-gray-500 mt-4">
              Your assessment will adapt to your ability level
            </div>
          </div>
        </div>
      </div>
  ...
  // FIXED: Always use liveResponses for difficulty data since backend doesn't include difficulty in results
  const difficultyData = generateDifficultyData(
    liveResponses,  // Always use liveResponses which has difficulty tracked
      ...
      <div className="max-w-7xl mx-auto p-6">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  ...
                  <div className="mb-6 p-4 bg-gray-700/50 rounded-lg">
                    ...
                    {userStats.proficiencies.map(prof => (
                      <div key={prof.subject} className="text-sm text-gray-400">
                        {prof.subject}:  {getThetaTierLabel(displayTheta)} (θ={prof.theta.toFixed(2)})
                        {/*{prof.tier}*/}
          ...
          <div className="space-y-4">
            {/* Proficiency Legend - Made Compact */}
            <div className="bg-gray-800 rounded-lg p-4 border border-gray-700">
              <h3 className="text-xs font-medium mb-2 text-gray-200">Proficiency Levels</h3>
